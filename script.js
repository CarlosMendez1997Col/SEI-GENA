...
function addMessage(role, content, id = null) {
    ...
    wrapper.innerHTML = `
        ...
        <div class="max-w-[80%] ${isBot ? 'bg-slate-900/50 border-slate-800' : 'bg-green-700 text-white border-green-600'} border p-4 rounded-2xl shadow-sm text-sm leading-relaxed">
            ${content}
        </div>
    `;
...
async function handleAction() {
    ...
    try {
        const response = await fetch('http://34.95.136.6:8000/ask', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_question: query }) // Limpio de passwords
        });

        const data = await response.json();
        const botDiv = document.getElementById(botMsgId).querySelector('.max-w-\\[80\\%\\]');

        if (data.response) {
            botDiv.innerHTML = data.response;
            updateInspector(data.metadata.titulo, data.metadata.institucion, "Análisis de licitación completado.");
        }
    } catch (error) {
